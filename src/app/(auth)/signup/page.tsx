...
export default function SignupPage() {
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      toast.success("Welcome to Raffle Rumble! 🎉");
      router.push("/");
    } catch (err: unknown) {
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="btn-gold flex items-center justify-center gap-2 rounded-xl bg-gold-500 py-3 font-bold text-black transition hover:bg-gold-400 disabled:opacity-60"
          >
            {loading ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-black/30 border-t-black" /> : "Create Account"}
          </button>
        </form>
